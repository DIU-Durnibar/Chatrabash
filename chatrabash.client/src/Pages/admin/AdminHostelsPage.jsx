...
export default function AdminHostelsPage() {
  ...
  const toggle = async (id, next) => {
    const res = await apiPatch(`/api/superadmin/hostels/${id}/status`, next);
                  ...
                  <div className="flex min-w-0 flex-1 items-start gap-3">
                    ...
                    <p className="mt-1 text-xs text-slate-600">
                      ...
                      {h.managerPhone ? (
                        <>
                          {" "}
                          · <span className="font-mono">{h.managerPhone}</span>
                        </>
                      ) : null}
                    </p>
                    </div>
                  ...
                  <div className="flex shrink-0 flex-wrap items-center gap-2">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-bold ${
                        h.status === "Active" ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"
                      }`}
                    >
                      {h.status}
                    </span>
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        toggle(h.id, h.status !== "Active");
                      }}
                      className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-50"
                    >
                      {h.status === "Active" ? "স্থগিত" : "সক্রিয়"}
                    </button>
                    <span className="text-xs font-semibold text-[var(--cb-primary)] group-open:hidden">বিস্তারিত ▾</span>
                    <span className="hidden text-xs font-semibold text-[var(--cb-primary)] group-open:inline">সংকুচিত ▴</span>
                  </div>
                </summary>
